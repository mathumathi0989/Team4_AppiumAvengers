...
public void user_already_watched_downloads() throws Exception {
	...
	testContext.getwatchVideoCleanerPage().allowPop();
  	testContext.getSubscribePage().clickBack();
  	actualDownloads = testContext.getwatchVideoCleanerPage().getDownloadsNo();
 // 	testContext.getSubscribePage().clickBack();
}
...
public void click_on_watched_video_cleaner_and_click_on_clean() {
	//testContext.getwatchVideoCleanerPage().isTrending();
...
public void toast_message_should_be_displayed_for_bookmarks() {
	...
	logger.info("Bookmarks are removed using Watched video cleaner");
}


@Then("Toast message should be displayed for downloads")
public void toast_message_should_be_displayed_for_downloads() {
	expectedDownloads = testContext.getwatchVideoCleanerPage().getDownloadsNo();
	Assert.assertNotEquals(actualDownloads, expectedDownloads);
	logger.info("Downloads are removed using Watched video cleaner");
}
